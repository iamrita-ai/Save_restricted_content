...
batch_states: Dict[int, Dict[str, Any]] = {}     # 'step', 'link', 'task_id', 'is_private', 'use_user_session', 'dest_chat_id'
batch_tasks: Dict[int, asyncio.Task] = {}

settings_states: Dict[int, str] = {}             # 'await_chat_id', 'await_remove_words'
...
def apply_remove_words(text: str, words: List[str]) -> str:
...
async def get_user_doc(user_id: int) -> Dict[str, Any]:
    ...
    if "history" not in doc:
        updates["history"] = []
    if "remove_words" not in doc:
        updates["remove_words"] = []
...
async def cmd_start(client: Client, msg: Message):
    ...
    if START_IMAGE_URL and msg.chat.type == "private":
    ...
    await log_to_channel(f"/start by {user.id} (@{user.username}) in {msg.chat.id}")
...
async def cmd_help(client: Client, msg: Message):
    ...
    text = (
        ...
        "• /help – Ye help menu\n"
        "• /login – DM me login menu (Session / Phone+OTP / QR Code)\n"
        "• /logout – Session logout (DM only)\n"
    ...
    await log_to_channel(f"/help by {user.id} in {msg.chat.id}")
...
async def cmd_status(client: Client, msg: Message):
    ...
    if prem:
        ...
        if isinstance(exp, datetime):
            prem_text = f"✅ YES (till {exp.strftime('%Y-%m-%d %H:%M:%S %Z')})"
        else:
            prem_text = "✅ YES"
    ...
    await msg.reply_text(text)
    await log_to_channel(f"/status by {user_id} in {msg.chat.id}")


# ---------- /plan ----------
@bot.on_message(filters.command("plan") & (filters.private | filters.group))
async def cmd_plan(client: Client, msg: Message):
    ...
    if created_at:
        text += f"📅 First seen: {created_at.strftime('%Y-%m-%d %H:%M:%S %Z')}\n"
    if last_seen:
        text += f"🕒 Last seen: {last_seen.strftime('%Y-%m-%d %H:%M:%S %Z')}\n"
    ...
    await msg.reply_text(text)
    await log_to_channel(f"/plan by {user_id} in {msg.chat.id}")
...
async def cmd_cancel(client: Client, msg: Message):
    ...
    if cancelled_any:
        ...
        await log_to_channel(f"/cancel used by {user_id} in {msg.chat.id}")
    else:
...
async def cmd_settings(client: Client, msg: Message):
    ...
    kb = InlineKeyboardMarkup(
        [
            ...
            [
                InlineKeyboardButton(
                    "🧽 Remove Words", callback_data="remove_words"
                )
            ],
        ]
    )
...
async def cmd_login(client: Client, msg: Message):
    ...
    text = (
        ...
        "aapka account (jisse aapne /login kiya) us channel/group me joined ho.\n"
        "Bot ko wahan member/admin hone ki zaroorat nahi. ✨"
    )
...
# ---------- /batch (DM + Groups) ----------
@bot.on_message(filters.command("batch") & (filters.private | filters.group))
async def cmd_batch(client: Client, msg: Message):
    print(f"[DEBUG] /batch from {msg.from_user.id} in {msg.chat.id}")
    user = msg.from_user
    if not user:
        return
    user_id = user.id
    ...
    dest_chat_id = msg.chat.id
    batch_states[user_id] = {"step": "wait_link", "link": None, "dest_chat_id": dest_chat_id}
    ...
    await log_to_channel(f"/batch started by {user_id} in {dest_chat_id}")
...
async def on_callback(client: Client, cq: CallbackQuery):
    ...
    if data == "reset_settings":
        await unset_user_fields(user_id, ["set_chat_id", "replace_serena", "remove_words"])
    ...
    if data == "remove_words":
        settings_states[user_id] = "await_remove_words"
        await cq.message.reply_text(
            "🧽 Remove Words\n\n"
            "Comma se separated words bhejiye jo aap text/caption se hatwana chahte hain.\n"
            "Example: Serena,Kumari,File\n\n"
            "Ye words case-insensitive tareeke se remove kiye jayenge."
        )
        await cq.answer("Remove words set karne ke liye list bhejiye. 💕")
        return

    # Login callbacks (DM only)
...
async def handle_phone_number(msg: Message):
    ...
    try:
        ...
        await msg.reply_text(
            ...
            "Kabhi-kabhi ye SMS ke bajaye 'Telegram' app ke andar hi dikhta hai "
            "(Login code chat ya notification me).\n"
        ...
        await msg.reply_text(f"Telegram ne code send karte waqt error diya: {e}")
        try:
            await user_client.disconnect()
        except Exception:
            pass
...
# ---------- /batch helper: handle_batch_link ----------
async def handle_batch_link(msg: Message):
    ...
    dest_chat_id = state.get("dest_chat_id", msg.chat.id)
...
# ---------- /batch helper: handle_batch_count ----------
async def handle_batch_count(msg: Message):
    ...
    # Agar session hai -> user-session se clone (ANY chat jahan user member ho)
    ...
    if use_user_session:
        task = asyncio.create_task(batch_worker_private(user_id, dest_chat_id, link, count, task_id))
    else:
        task = asyncio.create_task(batch_worker_public(user_id, dest_chat_id, link, count, task_id))
...
async def handle_remove_words(msg: Message):
    user_id = msg.from_user.id
    text = (msg.text or "").strip()

    if not text:
        await msg.reply_text(
            "Koi words nahi mile. Agar remove words clear karne hain to `reset` likh kar bhejo, "
            "warna comma se separated words bhejo. 💕"
        )
        return

    if text.lower() in ("reset", "clear"):
        await set_user_field(user_id, "remove_words", [])
        settings_states.pop(user_id, None)
        await msg.reply_text("Remove words list clear kar di gayi hai. 🌸")
        return

    parts = [w.strip() for w in text.split(",") if w.strip()]
    await set_user_field(user_id, "remove_words", parts)
    settings_states.pop(user_id, None)
    await msg.reply_text(
        "Remove words list set ho gayi hai:\n" + ", ".join(parts)
    )
...
async def on_plain_text(client: Client, msg: Message):
    ...
    if settings_states.get(user_id) == "await_remove_words":
        await handle_remove_words(msg)
        return
...
# ---------- BATCH HEADER UPDATE (DM me pinned romantic msg) ----------
async def update_batch_header_msg(
    user_id: int,
    header_msg: Message,
    link: str,
    done: int,
    ...
    text = (
        "💞 SERENA Batch Love Story 💞\n\n"
        f"📎 Source: {link}\n"
        f"📊 Progress: {done}/{total} messages cloned 💌\n"
        f"💫 Status: {status}\n"
    )
    kb = InlineKeyboardMarkup(
        [[InlineKeyboardButton("💌 Contact Owner", url="https://t.me/technicalserena")]]
...
async def process_one_message(
    ...
    """
    Har message ko clone karta hai dest_chat_id me:
    - TEXT + links as text
    - PHOTO as photo (fallback doc if PHOTO_EXT_INVALID)
    - VIDEO as video
    - DOCUMENT as document (PDF filename + ' Serena')
    - ANIMATION as gif
    - STICKER as sticker
    ...
    text = apply_remove_words(text, remove_words)
    if not text:
        text = "(empty message)"
    ...
    if has_media:
        ...
        try:
            ...
            if not file_path or not os.path.exists(file_path) or not os.path.isfile(file_path):
                ...
                try:
                    ...
                    # PDF ke liye filename change
                    pdf_name = None
                    if src_msg.document and src_msg.document.file_name:
                        orig = src_msg.document.file_name
                        name, ext = os.path.splitext(orig)
                        if ext.lower() == ".pdf":
                            pdf_name = f"{name} Serena{ext}"
                    ...
                    if src_msg.photo:
                        ...
                        extra_kwargs = {}
                        if pdf_name:
                            extra_kwargs["file_name"] = pdf_name
                        sent = await bot.send_document(
                            chat_id=dest_chat_id,
                            document=file_path,
                            caption=text if text != "(empty message)" else None,
        ...
        try:
            sent = await bot.send_message(dest_chat_id, text)
            if sent:
                sent_msgs.append(sent)
                downloaded_count_ref[0] += 1
        except RPCError:
...
async def batch_worker_private(user_id: int, dest_chat_id: int, link: str, count: int, task_id: str):
    ...
    try:
        ...
        replace_flag = bool(doc.get("replace_serena", False))
        remove_words = doc.get("remove_words") or []
        ...
        status = "completed"
        await bot.send_message(dest_chat_id, "Batch complete ho gaya. 🌸")
        await update_batch_header_msg(
            user_id,
            header,
            link,
        ...
        await log_to_channel(
            f"[USER_SESSION] Batch completed for user {user_id} | requested={count} | "
            f"downloaded={downloaded_count[0]} | errors={error_count[0]}"
        )

    except asyncio.CancelledError:
        status = "cancelled"
        try:
            await bot.send_message(dest_chat_id, "Batch cancel kar diya gaya. ❌")
        except Exception:
            pass
    except Exception as e:
        status = "error"
        try:
            await bot.send_message(dest_chat_id, f"Batch me error aaya: {e}")
        except Exception:
            pass
...
async def batch_worker_public(user_id: int, dest_chat_id: int, link: str, count: int, task_id: str):
    ...
    try:
        ...
        header = await bot.send_message(
            user_id,
            "🔰 SERENA Batch Start ho raha hai (Public)...\nDekhte hain kitni yaadein la sakti hoon. 💞",
        )
        try:
            await bot.pin_chat_message(user_id, header.id, disable_notification=True)
        except RPCError:
        ...
        status = "completed"
        await bot.send_message(dest_chat_id, "Public batch complete ho gaya. 🌸")
        await update_batch_header_msg(
            user_id,
            header,
            link,
        ...
        await log_to_channel(
            f"[BOT_PUBLIC] Batch completed for user {user_id} | requested={count} | "
            f"downloaded={downloaded_count[0]} | errors={error_count[0]}"
        )
