...
batch_states: Dict[int, Dict[str, Any]] = {}     # 'step', 'link', 'task_id', 'is_private'
...
async def cmd_start(client: Client, msg: Message):
    ...
    intro = (
        ...
        "📦 private / public channels se safely sab kuch recover karo.\n\n"
        "Bas yaad rakho…\n"
...
async def cmd_help(client: Client, msg: Message):
    text = (
        "🌹 *SERENA – Help Menu* 🌹\n\n"
        "Commands jo tumhe mere kareeb le aayenge:\n\n"
        "• /start – Romantic welcome & intro 💌\n"
        "• /help – Ye pyaara sa help menu\n"
        "• /login – Login menu (Session / Phone+OTP / QR Code)\n"
        "• /logout – Apne account ka session hata do\n"
        "• /batch – Channel/chat link se batch me files/messages nikaalo\n"
        "• /status – Tumhara login, premium aur current status ❤️\n"
        "• /plan – Tumhari history, stats, premium remaining time\n"
        "• /settings – Set Chat ID, 'Serena' → 'Kumari' rename option\n"
        "• /cancel – Abhi chal raha kaam rok do (login/batch etc.)\n\n"
        "💘 Login Methods:\n"
        "1️⃣ *Session String* – Direct Pyrogram/Telethon session paste karo.\n"
        "2️⃣ *Phone + OTP* – 10-digit number + OTP (e.g. `4 2 1 5`).\n"
        "   ⚠️ Agar OTP kisi bot/chat me share karoge to Telegram us code ko block kar sakta hai.\n"
        "3️⃣ *QR Code Login* – Safe & sweet method, recommended. 💞\n\n"
        "👑 Owner-only:\n"
        "• /addpremium user_id days – Premium do (e.g. `/addpremium 123456789 12`)\n"
        "• /remove user_id – Premium hatao\n\n"
        "Note:\n"
        "• India number: sirf 10 digit, +91 main khud jod dungi.\n"
        "• Har action ka log secure logs channel me bhi jata hai. 🔍"
    )
...
async def cmd_addpremium(client: Client, msg: Message):
    ...
    if len(parts) < 3:
        await msg.reply_text("Use: `/addpremium user_id days`\nExample: `/addpremium 123456789 12`", parse_mode="markdown")
        return
...
async def cmd_remove_premium(client: Client, msg: Message):
    ...
    parts = msg.text.strip().split()
    if len(parts) < 2:
        await msg.reply_text("Use: `/remove user_id`\nExample: `/remove 123456789`", parse_mode="markdown")
        return
    ...
    await msg.reply_text(f"User `{target_id}` se premium hata diya gaya hai. 💔", parse_mode="markdown")
...
async def cmd_login(client: Client, msg: Message):
    ...
    text = (
        ...
        "1️⃣ *Session String* – Pyrogram/Telethon session paste karo (advanced lovers 💫).\n"
        "2️⃣ *Phone + OTP* – 10-digit number + OTP (e.g. `4 2 1 5`).\n"
        "   ⚠️ OTP kisi ke sath share mat karo, warna Telegram us code ko block kar sakta hai.\n"
        "3️⃣ *QR Code Login* – Sabse safe & romantic method. 💞\n"
    )
...
async def on_callback(client: Client, cq: CallbackQuery):
    ...
    if data == "login_session":
        ...
        await cq.message.reply_text(
            ...
            "_Ye option sirf tab use karein jab aapko session string ka concept pata ho._",
            parse_mode="markdown",
        )
...
async def handle_session_login(msg: Message):
    ...
    try:
        ...
        await msg.reply_text(f"Session invalid ya login error: `{e}`", parse_mode="markdown")
    finally:
        try:
...
async def handle_phone_number(msg: Message):
    ...
    if not re.fullmatch(r"\d{10}", text):
        await msg.reply_text("Please valid *10-digit* mobile number bhejiye (sirf digits). 💕", parse_mode="markdown")
        return
    ...
    try:
        ...
        await msg.reply_text(
            ...
            "⚠️ Dhyaan rahe: OTP kisi aur bot/chat me share karoge to Telegram code block kar sakta hai.",
            parse_mode="markdown",
        )
        ...
        await msg.reply_text(f"Login start me error: `{e}`", parse_mode="markdown")
        try:
            await user_client.disconnect()
        except Exception:
            pass
...
async def handle_phone_code(msg: Message):
    ...
    try:
        ...
        await msg.reply_text(f"Sign-in error: `{e}`", parse_mode="markdown")
        try:
            await user_client.disconnect()
        except Exception:
            pass
    ...
    try:
        ...
        await msg.reply_text(f"Session export me error: `{e}`", parse_mode="markdown")
        try:
            await user_client.disconnect()
        except Exception:
            pass
    ...
    try:
        ...
        while True:
            ...
            try:
                ...
                await bot.send_message(chat_id, f"QR login me error: `{e}`", parse_mode="markdown")
                return

        try:
            session_string = await user_client.export_session_string()
        except Exception as e:
            await bot.send_message(chat_id, f"Session export me error: `{e}`", parse_mode="markdown")
            return
...
async def cmd_batch(client: Client, msg: Message):
    ...
    await msg.reply_text(
        ...
        "Public link ke liye login ki zarurat nahi hai.\n"
        "Private link ke liye pehle /login karna hoga. 🔐",
        parse_mode="markdown",
    )
...
async def handle_batch_count(msg: Message):
    ...
    if count > MAX_BATCH_LIMIT:
        await msg.reply_text(
            f"Max limit {MAX_BATCH_LIMIT} hai. Main {MAX_BATCH_LIMIT} messages tak hi le paungi. 💫"
        )
    ...
    if is_private:
        # Private channel/group (/c/…) -> session required
        doc = await get_user_doc(user_id)
...
async def handle_settings_chat_id(msg: Message):
    ...
    try:
        ...
        await msg.reply_text("Chat ID integer hona chahiye (example: `-1001234567890`). 💕", parse_mode="markdown")
        return

    await set_user_field(user_id, "set_chat_id", chat_id)
    settings_states.pop(user_id, None)
    await msg.reply_text(f"Set Chat ID saved: `{chat_id}` ✅", parse_mode="markdown")
...
async def process_one_message(
    ...
    if src_msg.media:
        ...
        # Safety: file must exist & be a file
...
# ---------- BATCH WORKER – PRIVATE (user session) ----------
async def batch_worker_private(user_id: int, link: str, count: int, task_id: str):
    ...
    try:
        ...
        await log_to_channel(
            f"[PRIVATE] Batch completed for user {user_id} | requested={count} | downloaded={downloaded_count[0]} | errors={error_count[0]}"
        )
        ...
        status = "error"
        try:
            await bot.send_message(user_id, f"Batch me error aaya: `{e}`", parse_mode="markdown")
        except Exception:
            pass
        await log_to_channel(f"[PRIVATE] Batch error for user {user_id}: {e}")
    finally:
...
# ---------- BATCH WORKER – PUBLIC (bot session) ----------
async def batch_worker_public(user_id: int, link: str, count: int, task_id: str):
    ...
    try:
        ...
        # yaha src_client = bot (public access)
        src_client = bot
        ...
        await log_to_channel(
            f"[PUBLIC] Batch completed for user {user_id} | requested={count} | downloaded={downloaded_count[0]} | errors={error_count[0]}"
        )
        ...
        try:
            ...
            pass
        await log_to_channel(f"[PUBLIC] Batch error for user {user_id}: {e}")
    finally:
        await finalize_batch_record(
            user_id, task_id, status, downloaded_count[0], error_count[0], media_count[0]
        )
